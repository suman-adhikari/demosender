        ...
        return this.each(function () {
            ...
            var self = {
                ...
                setInputValue: function () {
                    ...
                    var tdselectable = ".ui-state-active:not(.ui-datepicker-unselectable)";

                    if ($container.find('td ' + tdselectable).length == 0) {
                        $container.find('td .ui-state-highlight').addClass('ui-state-active');
                    }

                    var $activeCell = $container.find('td a' + tdselectable);
                    ...
                    if (todayDate.getDate() == selectedDay && todayDate.getMonth() == parseInt(selectedMonth - 1) && todayDate.getFullYear() == selectedYear) {

                        var Hour = $activeCell.closest('.dateTimePicker').find('.ui-tpicker-hour').text();
                        var Minute = $activeCell.closest('.dateTimePicker').find('.ui-tpicker-min').text();

                        if (parseInt(Hour.substring(0, Hour.length - 1)) < todayDate.getHours()) {

                            $activeCell.closest('.dateTimePicker').find('.ui-tpicker-hour').text(todayDate.getHours() + ":");
                        ...
                        if (((parseInt(Hour.substring(0, Hour.length - 1)) <= todayDate.getHours())) && (parseInt(Minute.substring(0, Minute.length - 1)) < todayDate.getMinutes())) {


                            $activeCell.closest('.dateTimePicker').find('.ui-tpicker-min').text((todayDate.getMinutes() + 1) + ":");
                            $input.val(selectedYear + "-" +
                            selectedMonth + "-" + selectedDay + " " + $activeCell.closest('.dateTimePicker').find('.ui-tpicker-hour').text() +
                            $activeCell.closest('.dateTimePicker').find('.ui-tpicker-min').text() + $activeCell.closest('.dateTimePicker').find('.ui-tpicker-sec').text()).change();
                            $input.trigger('click');
                        ...
                        //$input.val((selectedYear + "-" + selectedMonth + "-" + selectedDay + " " + todayDate.getHours() + ":" + parseInt(todayDate.getMinutes() + 1) + ":" + todayDate.getSeconds()));
                    ...
                    for (i = 0, count = 1; i < weeks; i++) {
                        ...
                        for (var j = 0; j < 7; ++j, ++count) {
                            ...
                            if (count > firstDay.getDay() && count <= totalDays + firstDay.getDay()) {
                                ...
                                var anchorClass = " ui-state-default ";

                                if (typeof options != 'undefined' && typeof options.minDate != 'undefined') {
                                    ...
                                    } else if (typeof(options.minDate)) {

                                        var todayDates = new Date(options.minDate.val());

                                        var todayDate = new Date(todayDates.getFullYear() + '/' + (todayDates.getMonth() + 1) + '/' + parseInt(todayDates.getDate()));

                                        // todayDate.setDate(todayDate.getDate() - 1);

                                    } else {
                                        var todayDate = new Date(options.minDate);
                                    ...
                                    var calenderDate = (firstDay.getFullYear() + '/' + (firstDay.getMonth() + 1) + '/' + parseInt(count - firstDay.getDay()));


                                    var CDate = new Date(calenderDate);

                                    if (CDate < todayDate) {
                                        anchorClass += " ui-datepicker-unselectable ui-state-disabled";
                                    }

                                }
                                if (typeof options != 'undefined' && typeof options.maxDate != 'undefined') {


                                    if (options.maxDate != 0) {

                                        var todayDate = new Date(options.maxDate);
                                ...
                                if ((count - firstDay.getDay()) == new Date().getDate() && currentDate.getMonth() == new Date().getMonth() && currentDate.getFullYear() == new Date().getFullYear()) {
                                    $td.html("<a class='" + anchorClass + " ui-state-highlight'>" + parseInt(count - firstDay.getDay()) + "</a>");
                                    $tr.append($td);
                                }
                                else if ($input.val() != '') {
                                    if (count - firstDay.getDay() == new Date($input.val().split(' ')[0]).getDate() && firstDay.getMonth() == new Date($input.val().split(' ')[0]).getMonth()
                                        ...
                                        $td.html("<a class='" + anchorClass + " ui-state-active'>" + parseInt(count - firstDay.getDay()) + "</a>");
                                        $tr.append($td);
                                    }
                                    else {
                                        if (count - firstDay.getDay() == 1 && (firstDay.getMonth() != new Date($input.val().split(' ')[0]).getMonth()
                                            ...
                                            $td.html("<a class='" + anchorClass + "'>" + parseInt(count - firstDay.getDay()) + "</a>");
                                            $tr.append($td);
                                        }
                                    }
                                }
